...
function instructionsFile(
  ...
  const body = [
    ...
    "",
    "**Response style:** concise by default — announce the path briefly, pause at gates,",
    "and end with a compact summary (changed files · verify verdict · next action ·",
    "one-line spend). Keep detail in artifacts (PLAN / REVIEWS / VERIFY) and",
    "`/flow-status`; don't reprint per-phase spend tables or STATE diffs. Every command",
    "honors `--verbose` for the full phase-by-phase narration.",
    "",
